...
#         surf = ax1.plot_surface(X[:,:,0], Y[:,:,0], u_pinn[:,:,t_idx], cmap='seismic')
...
#         surf = ax2.plot_surface(X[:,:,0], Y[:,:,0], u_numerical[:,:,t_idx], cmap='seismic')
...
def main():
    ...
    model = load_model(model_path)
    predictions = get_pinn_predictions(model, X_flat, Y_flat, T_flat)
